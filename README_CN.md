...
*   ✅ **流式输出支持**：支持实时流式显示 AI 响应，通过可选的 `--stream` 参数在流式和非流式模式之间切换，获得更快的反馈体验。
...
*   ✅ **内置网页搜索**：多种搜索工具包括百度搜索（中文优化）、通用搜索、学术搜索、新闻搜索、电商搜索、社交媒体搜索和技术搜索。所有工具均支持多引擎备用链，无需 API Key，开箱即用。
...
```bash
# 交互模式开启流式输出（推荐，用于实时反馈）
mini-agent --stream
mini-agent -s

# 非交互模式开启流式输出
mini-agent --stream --task "你的任务"
mini-agent -s -t "你的任务"
...
| 模式 | 参数 | 行为 | 适用场景 |
|------|------|------|----------|
| **流式** | `--stream` 或 `-s` | AI 响应实时显示，逐字呈现 | 交互式对话、长响应 |
| **非流式** | （默认） | 等待完整响应后统一显示 | 脚本化任务、安静环境 |

**流式模式特点：**
- 实时显示 AI 思考过程（如模型支持）
- 逐字增量显示生成的文本
- 工具调用结果在流式完成后显示
- 与非流式模式功能完全相同，但响应感知更快
