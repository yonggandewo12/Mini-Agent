...
项目在 `mini_agent/tools/search/` 目录下提供了**搜索工具框架**，支持多种专业搜索工具和多引擎备用链。

#### 可用的搜索工具

| 工具 | 用途 | 引擎链 |
|------|------|--------|
| `GeneralSearchTool` | 通用网页搜索 | Bing → 360搜索 → 搜狗 → DuckDuckGo → **百度** |
| `AcademicSearchTool` | 学术论文搜索 | Semantic Scholar → arXiv → **百度** |
| `NewsSearchTool` | 新闻文章搜索 | Bing新闻 → Google新闻 RSS → DuckDuckGo → **百度** |
| `EcommerceSearchTool` | 电商商品搜索 | Bing(Taobao) → Bing(JD) → 搜狗 → **百度** |
| `SocialSearchTool` | 社交媒体搜索 | Bing(Weibo) → Bing(WeChat) → 搜狗 → **百度** |
| `TechSearchTool` | 技术问答搜索 | Bing(StackOverflow) → Bing(GitHub) → 搜狗 → **百度** |

> **注意：** 百度搜索作为所有搜索工具的**最终兜底**，确保在其他引擎失效时仍有最大覆盖率。

#### BaseSearchTool 特性

所有搜索工具都继承自 `BaseSearchTool`，提供以下功能：
- **异步 HTTP 请求**（使用线程池）
- **浏览器 User-Agent 伪装**，避免被反爬
- **重试装饰器**（3次重试，延迟递增）
- **速率限制**，防止请求过于频繁
- **HTML 解析**（使用 BeautifulSoup）
- **多引擎备用链** - 主引擎失败时自动切换到备用引擎
...
在 `cli.py` 的 `add_workspace_tools()` 函数中注册：
