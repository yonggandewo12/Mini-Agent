...
class EcommerceSearchTool(BaseSearchTool):
    """E-commerce product search tool.

    Suitable for finding products, prices, and shopping information.
    Uses search engines with site-specific operators to find product info.
    """

    # Search engine configurations for e-commerce search
    ENGINES = [
        {
            "name": "Bing Taobao",
            "url_template": "https://www.bing.com/search?q=site%3Ataobao.com+OR+site%3Atmall.com+{query}&count={max_results}",
        ...
        {
            "name": "Bing JD",
            "url_template": "https://www.bing.com/search?q=site%3Ajd.com+{query}&count={max_results}",
            "result_selector": ".b_algo",
            "title_selector": "h2 a",
            "abstract_selector": ".b_paractl",
        },
        {
            "name": "Sogou",
            "url_template": "https://www.sogou.com/web?query=taobao.com+OR+tmall.com+OR+jd.com+{query}&num={max_results}",
    ...
    def __init__(self, rate_limit: float = 1.0, timeout: int = 10):
        """Initialize e-commerce search tool."""
        super().__init__(rate_limit=rate_limit, timeout=timeout)

    @property
    def name(self) -> str:
        """Tool name."""
        return "EcommerceSearch"

    @property
    def description(self) -> str:
        """Tool description."""
        return "搜索电商平台的产品、价格和购物信息。"
    ...
    async def _execute_search(self, query: str, max_results: int, **kwargs) -> list[dict]:
        """Execute e-commerce search using search engines.

        Engine fallback chain:
        - Bing Taobao/Tmall (主要)
        - Bing JD (备用)
        - Sogou (备用)
        - Baidu (最终兜底)
        """
        ...
        # Final fallback: Baidu e-commerce search
        try:
    ...
    async def _search_baidu(self, query: str, max_results: int) -> list[dict]:
        ...
        encoded_query = urllib.parse.quote(f"购物 {query}")
    ...
    def _search_with_engine(self, query: str, max_results: int, engine: dict) -> list[dict]:
        ...
        for item in result_items[:max_results]:
            try:
                ...
                if "taobao.com" in raw_url or "tmall.com" in raw_url:
                    source = "淘宝/天猫"
                elif "jd.com" in raw_url:
                    source = "京东"
                else:
                    source = "电商平台"
